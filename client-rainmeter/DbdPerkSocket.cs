...
namespace DbdPerkSocket
{
    class Measure
    {
        ...
        {
            if (!string.IsNullOrEmpty(bang) && api != null)
            {
        ...
        internal string GetPerk(int index)
        {
            lock (sync)
            {
                if (index >= 1 && index <= 4) return perkUrls[index - 1];
                return "";
            }
        }
    ...
    public class Plugin
    {
        ...
        [DllExport]
        public static string Perk(IntPtr data, int argc, string[] argv)
        {
            Measure measure = (Measure)data;
            int index = 0;
            if (argc >= 1)
